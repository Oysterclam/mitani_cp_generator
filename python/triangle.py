...
def get_angle_list(graph, points):
    angle_list = []
    for i in range(len(graph)):
        ...
        for j in range(neighbors):
            ...
            angles.append(((math.atan2(x2,y2)-math.atan2(x1,y1))*180/math.pi)%180)
        angle_list.append(angles)
    return angle_list

def rotation_ordered_neighbors(G,points,s):
    #returns the neighboring vertices in counterclockwise order
...
class crease_pattern(Frame):
    ...
    def initUI(self):
        ...
        #doing angles 
        angle_list = get_angle_list(graph, points)
